...
```
npx http-server -p 8181 .   # then open http://127.0.0.1:8181/
```

Sized for a phone — use a mobile viewport (iPhone 14 Pro, 393 × 852). It is
installable to the home screen: `manifest.webmanifest` plus the iOS icons and
startup images under `assets/`, generated by `tools/generate-assets.mjs`.

The earlier GHA DISCOVERY sign-in mockup now lives at `signin/`, with its own
manifest.

`dev/` holds a separate, forward-looking prototype: a redesigned settlement
